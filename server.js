...
app.get('/', async function(req, res){
  const noticePage = parseInt(req.query.noticePage) || 1;
  const qnaPage = parseInt(req.query.qnaPage) || 1;
  const pageSize = 2; // 페이지당 게시글 수를 2로 설정
  ...
  // Paginate the adminPosts
  const paginatedAdminPosts = adminPosts.slice((noticePage - 1) * pageSize, noticePage * pageSize);
  const totalAdminPosts = adminPosts.length;
  const totalAdminPages = Math.ceil(totalAdminPosts / pageSize);

  // Paginate the userPosts
  const paginatedUserPosts = userPosts.slice((qnaPage - 1) * pageSize, qnaPage * pageSize);
  const totalUserPosts = userPosts.length;
  const totalUserPages = Math.ceil(totalUserPosts / pageSize);

  res.render('First.ejs', {
    user: req.user,
    noticePosts: paginatedAdminPosts,
    qnaPosts: paginatedUserPosts,
    currentNoticePage: noticePage,
    totalNoticePages: totalAdminPages,
    currentQnaPage: qnaPage,
    totalQnaPages: totalUserPages
  });
});
...
app.get('/search', 로그인했니, async (req, res) => {
  ...
  if (searchType === 'notice') {
    query = { 작성자: { $in: await db.collection('admin').distinct('_id') }, $or: [ { 제목: regex }, { 날짜: regex }, { 작성자이름: regex } ] };
  } else if (searchType === 'qna') {
    query = { 작성자: { $in: await db.collection('login').distinct('_id') }, $or: [ { 제목: regex }, { 날짜: regex }, { 작성자이름: regex } ] };
  } else {
    query = { $or: [ { 제목: regex }, { 날짜: regex }, { 작성자이름: regex } ] };
  }

  try {
    const posts = await db.collection('post').find(query)
    ...
    const isAdmin = req.user && req.user.userType === 'admin';
    res.render('search.ejs', { posts, isAdmin, user: req.user, searchValue, searchType, currentPage: page, totalPages: totalPages });
  } catch (err) {
    console.error(err);
    res.status(500).send('Internal Server Error');
  }
